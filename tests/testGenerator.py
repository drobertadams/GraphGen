...
class TestGenerator(unittest.TestCase):
    ...
    def testFindMatchingProductions(self):
        ...
        gen = Generator()
        self.assertEquals( len(gen._findMatchingProductions(g, [p])), 0)        

        # One matching production, a simple vertex "A".
        # Graph is A->B
        # Production is A ==> None
        g = Graph()
        g.addEdge(Vertex('g0', 'A'), Vertex('g1', 'B'))
        lhs = Graph()
        ...
        rhs = Graph()
        p = Production(lhs, rhs)
        allMatches = gen._findMatchingProductions(g, [p])
        self.assertEquals( len(allMatches), 1)
        match = allMatches[0]
        self.assertEquals(match[0], p) # the match should contain our production
        mapping = match[1]
        self.assertDictEqual(mapping, {'l0':'g0'}) # l0 on the LHS maps to g0 in the graph
        ...
        g.addEdge(Vertex('g0', 'A'), Vertex('g1', 'B'))
        lhs = Graph()
        lhs.addVertex(Vertex('l0', 'A', '2'))
        rhs = Graph()
        # p1 is A ==> None
        # p2 is also A ==> None
        # Both of these production's lhs can be found in g.
        p1 = Production(lhs, rhs)
        p2 = Production(lhs, rhs)
        allMatches = gen._findMatchingProductions(g, [p1, p2])
        self.assertEquals( len(allMatches), 2)
        self.assertEquals(allMatches[0][0], p1) # our first production matches
        self.assertEquals(allMatches[1][0], p2) # our second production matches
        self.assertDictEqual(allMatches[0][1], {'l0':'g0'}) 
        self.assertDictEqual(allMatches[1][1], {'l0':'g0'}) 

    #--------------------------------------------------------------------------
    def _testMapRHSToGraph(self):
    ...
    #--------------------------------------------------------------------------
    def _testDeleteMissingEdges(self):
    ...
    #--------------------------------------------------------------------------
    def _testDeleteMissingVertices(self):
...
#--------------------------------------------------------------------------
    ...
    #--------------------------------------------------------------------------
    def _testGenerate_Blackbox(self):
...
#--------------------------------------------------------------------------
    def _testGenerate_Blackbox2(self):
    ...
    #--------------------------------------------------------------------------
    def _testGenerate_Blackbox3(self):
...
#--------------------------------------------------------------------------
    def _testGenerate_Blackbox4(self):
...
"""
    ...
    #--------------------------------------------------------------------------
    def _testGenerateFromFile(self):
        gen = Generator()
